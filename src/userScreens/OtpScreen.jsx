    ...
    <Navbar />
      ...
      exit={{x: window.innerWidth, transition:{duration :0.1} }}>
          <div className="  w-screen md:w-600  text-center flex flex-col justify-between">
            ...
            <form className="h-614 flex flex-col justify-between" onSubmit={otpVerify} >
            <div className="pt-5">
            
            {open ? 
              <input type="text" onChange={(e)=>{setOtp(e.target.value)}} className="placeholder:text-2xl border text-2xl rounded-2xl h-16 w-400 ml-5 outline-none focus:outline-dark-purple"  placeholder= 'Enter Password'></input>
               :
              <input type="number" onChange={(e)=>{setOtp(e.target.value)}} className="placeholder:text-2xl border text-2xl rounded-2xl h-16 w-400 ml-5 outline-none focus:outline-dark-purple"  placeholder= 'Enter Otp'></input>
              }
           
              <p></p>
                 { !open &&  <p className='text-start pl-36 pt-3 font-bold text-dark-purple  text-green-600' onClick={()=>setOpen(true)}>Use Password</p>}
            ...
            <div className="  ">
              <button className="bg-green-600 w-400 text-2xl text-white rounded-md h-16"  type="submit" >Submit</button>
            </div>
            </form>
