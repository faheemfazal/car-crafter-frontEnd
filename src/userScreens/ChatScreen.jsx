...
function ChatScreen() {
  ...
  useEffect(() => {
    ...
    getChatList(id, token).then((res) => {
      ...
      if (res.status === 200) {
        setLoader(false)
        console.log(res.data.chatlist.members);
        const data = res.data.chatlist
        // const reciverList = data.filter(order => order.members.includes(id))
        const filteredMembers = data.filter(member => member.memdersData[0]?._id !== id);

        console.log(filteredMembers, 'fdjkjsdbfsdkjn');
        ...
        <div className='lg:w-3/5 w-full h-45rem sm:flex bg-white'>
          ...
          <div className='sm:hidden  block w-full '>
            ...
            <div className='md:absolute sticky top-40  w- h-20 shadow-md flex justify-between pr-2 ' >
              ...
              <div className='w-20 h-20 bg-green-600 rounded-xl p-4 z-50' onClick={() => setOpenlist(true)}  >
                <FaUsers className='text-3xl font-bold m-2 text-white' />

              </div>
                { <Chatlist chatlist={chatlist} setChatData={setChatData} setOpenlist={setOpenlist} Open={openlist}/>}
          ...
          {chatdata ? (
            <Message name={name} setChatData={setChatData} chatdata={chatdata} senderId={id} token={token} />
          ) : (
            <>
              <div>
                ...
                <div className="md:hidden block text-red-800 pt-3 text-center h-8 capitalize">see all</div>
              </div>
            </>
          )
          }
