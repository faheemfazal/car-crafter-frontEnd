...
function PlaceSet({open,setOpen,sutable,location,setOpenDate,setCity}) {
  ...
  return (
    ...
    <div className='fixed left-0 right-0 bottom-0 top-0 ' onClick={()=>{setOpen(false)}} />
    ...
    <div className=" "  >
                <div className="w-9/12  flex h-12 border-slate-400 border-2 m-2 "  >
                   ...
                   <div>
                    <input type="text" className="w-6/6 mt-2 ml-2 h-8 bg-slate-100 outline-none focus:outline-white" placeholder="Select your starting point" />
                   </div>
                ...
                <div>
                 ...
                 {sutable.map((data,index)=>
                    (<div className='flex p-2 bg-slate-100'  onClick={()=> locationPick(data)}>
                        <GrMapLocation className='ml-3 mt-2 text-3xl text-black' />

                   <div className='ml-3'>
                    <h1 className='font-semibold text-lg'>{data.city},<span className='ml-3'>{data.state}</span></h1>
