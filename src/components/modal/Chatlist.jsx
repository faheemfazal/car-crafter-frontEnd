...
const closeWindow = () =>{
    ...
    return (
        <>
            ...
            <div className='fixed top-0 left-0 right-0 bottom-0  bg-black  opacity-80' id='container' onClick={closeWindow}/>
            <div className='fixed z-50   bg-white  top-0 left-0 w-72 bottom-0 border-2 border-stone-600 shadow-2xl '>
                {chatlist.map((data, index) =>
                (<div className='flex bg-gray-100 hover:bg-slate-400 w-full h-16 ' onClick={(e) => {
                    e.preventDefault();
                    setChatData(data)
                    closeWindow()
                }} >
