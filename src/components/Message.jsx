...
function Message({date,name,chatdata,senderId,token,setChatData}) {
    ...
    console.log(chatdata,'dfnjsdhfjksd..........f');
    ...
    <div className="flex flex-col gap-6 w-5/6 h-[646px] bg-gray-300   rounded-md p-4 justify-between ">
       ...
       (
        ...
        <div className="p-4 flex flex-col bg-white hover:bg-red-100">
         ...
         <img className='w-12 h-12' src="https://img.icons8.com/3d-fluency/94/null/user-male-circle.png"/>
...
<div className='flex flex-col gap-3 p-5 h-full  overflow-y-scroll'>
    {messagedata.length !==0  ? messagedata?.map((message)=>{
        return (
            <>
            <div 
             className={message.sender == senderId ? "  self-end rounded-t-lg rounded-l-lg bg-sender flex flex-col text-white bg-green-500" : " @apply text-[black] max-w-md w-fit flex flex-col gap-1 p-[-0.5rem] rounded-t-lg rounded-r-lg bg-white"}>
                <span className='text-sm font-semibold justify-center mx-3 my-2 ' >{message?.text}</span>
                <span className='text-xs justify-end flex mx-3 mb-1  ' >{moment(message.createdAt).format('h : mm A')}</span>
...
<input
    ...
    <div className="bg-primary rounded-lg pointer hover:border hover:border-primary hover:text-primary hover:bg-white border-2 border-gray-300" onClick={sendChat} ><h1 className='mx-2 my-2 text-black'>Send</h1></div>
